...
func run() error {
	...
	// NOTE: We must not block on lnd being fully synced to chain here. lnd's
	// block processing can be blocked on tapd's aux components (for example
	// on the aux sweeper, if a channel is being resolved on chain), and
	// those only become available once we configure and start the tapd
	// server below. Since lnd only reports itself as synced to chain once
	// its blockbeat dispatcher caught up, waiting for that here would
	// deadlock the startup. We do wait for the chain notifier to be ready
	// though, as tapd needs it to subscribe to new blocks.
