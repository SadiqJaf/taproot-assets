...
func (h *OrderHandler) subscribeHtlcs(ctx context.Context) error {
	...
	for {
		...
		case event := <-events:
			...
			// Once an HTLC reached a final state, we can forget that
			// we sent a resolution for it. We only ever resolve
			// forwards, as HTLCs we receive ourselves are handled
			// by lnd's invoice registry and never reach the
			// forwarding interceptor. We still do this before the
			// event type check below, so we also clean up if lnd
			// ever reports a resolved HTLC under a different type.
